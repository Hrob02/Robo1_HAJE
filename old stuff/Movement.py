...
#    Use `ros2 topic list` to confirm actual names, then set params below or via launch.
#
# Notes:
# - The subject README mentions the drone flies at a fixed altitude (spawn z). This controller
#   will still try to regulate z; if your model ignores z, it's fine (the z command is a no-op).
# - Collisions are disabled by default (per README): this node does not do avoidance.
